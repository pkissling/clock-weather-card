...
| hide_temperature_unit | boolean          | **Optional** | Hides the temperature unit letter (`C`/`F`) from all temperatures — the current temperature in the today section and every forecast row — keeping only the degree symbol (e.g. `21°` instead of `21°F`) | `false`   |
